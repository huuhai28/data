...
# ─── MYSQL ────────────────────────────────────────────────────docker compose up -d cloudbeaver
...
common::get_polaris_token() {
  ...
  for i in $(seq 1 20); do
    ...
    echo "  ... chờ Polaris (lần $i/5)..."; sleep 5
  done
...
common::cleanup_flink_jobs() {
  ...
  JOBS=$(docker exec flink-jobmanager curl -s http://localhost:8081/jobs \
    | python3 -c "import sys,json; [print(j['id']) for j in json.load(sys.stdin)['jobs'] if j['status']=='RUNNING']" 2>/dev/null || true)
...
common::create_polaris_catalog() {
  ...
  HTTP=$(curl -s -o /dev/null -w "%{http_code}" \
    ...
    -H "Content-Type: application/json" \
    -d "{\"catalog\":{\"type\":\"INTERNAL\",\"name\":\"$CATALOG_NAME\",\"storageConfigInfo\":{\"storageType\":\"S3\",\"allowedLocations\":[\"$S3_LOCATION\"],\"pathStyleAccess\":true},\"properties\":{\"default-base-location\":\"${S3_LOCATION}iceberg-data\"}}}")
  case "$HTTP" in
    201) echo "  ✅ Catalog '$CATALOG_NAME' đã được tạo." ;;
    409) echo "  ✅ Catalog '$CATALOG_NAME' đã tồn tại." ;;
...
# ─── SQL GENERATORS ───────────────────────────────────────────
# Các hàm này append DDL vào file SQL, dùng cho config-driven pipeline.
# Gọi trong vòng lặp để tự động sinh SQL từ TABLES_REGISTRY.

# common::gen_kafka_to_file <file> <table> <topic> <group> <pk> <cols>
common::gen_kafka_to_file() {
  local FILE="$1" TABLE="$2" TOPIC="$3" GROUP="$4" PK="$5" COLS="$6"
  cat >> "$FILE" <<SQL

-- Kafka source: ${TABLE}
DROP TABLE IF EXISTS ${TABLE}_kafka;
...
# common::gen_iceberg_to_file <file> <catalog> <namespace> <table> <pk> <cols>
common::gen_iceberg_to_file() {
  local FILE="$1" C="$2" NS="$3" TABLE="$4" PK="$5" COLS="$6"
  cat >> "$FILE" <<SQL

-- Iceberg sink: ${TABLE}
CREATE TABLE IF NOT EXISTS ${C}.${NS}.${TABLE} (
  ${COLS},
  PRIMARY KEY (${PK}) NOT ENFORCED
) WITH (
...
# common::gen_trino_raw_to_file <file> <namespace> <table> <s3_location> <cols>
common::gen_trino_raw_to_file() {
  local FILE="$1" NS="$2" TABLE="$3" S3_LOC="$4" COLS="$5"
  cat >> "$FILE" <<SQL
CREATE TABLE minio.${NS}.${TABLE} (
  ${COLS}
) WITH (
